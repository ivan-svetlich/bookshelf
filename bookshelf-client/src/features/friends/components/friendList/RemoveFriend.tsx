...
const RemoveFriend = ({show, handleClose, id, username, setUpdate}: RemoveFriendProps) => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        await friends.removeFriend(id).then(() => {
          setUpdate(true);
          dispatch(setMessage({content: `${username} was removed from your friendList`, variant: 'Success'}));
        });
      }
      catch {
        dispatch(setMessage({content: `Oops! we couldn't remove ${username} from your friendList. Please try again`, 
          variant: 'Danger'}));
      }
