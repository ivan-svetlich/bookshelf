...
const PendingRequests = () => {
    ...
    return (
        <div>
            <SubHeader title="Pending requests" childComp={() => FriendsNav('pending')} icon='fas fa-user-clock'></SubHeader>
            <div id="requests-page">
                ...
                friendListState.data.length > 0 ?
                    friendListState.data.map(friend => (<RequestCard friend={friend} setUpdate={setUpdate} />))
                : "No pending requests"}
            </div>
        </div>
    );
