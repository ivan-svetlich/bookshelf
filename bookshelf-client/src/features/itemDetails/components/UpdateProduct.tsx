...
const UpdateProduct = ({
  ...
  setUpdateProductInfo
    ...
    if(isNumeric(value)) {
      ...
      setInputs(values => ({...values, [inputName]: value.replace(',', '.')}));
    }
    else {     
    ...
    const handleSubmit = async () => {
      if(product) {
        if(inputs.price === null) {
          await handleUpdateProduct({id: product.id, price: product.price.toString()})
        }
    ...
    return (
        ...
        <form name="add_book">
          <Modal.Body>
              <table width="100%" cellSpacing="0" cellPadding="5">
                <tbody>
                  ...
                  <tr>
                    <td width="130" valign="top">Price</td>
                    <td>
                      <div className="input-group">
                        <div className="input-group-prepend">
                          <span className="input-group-text">$</span>
                        </div>
                        <input name='price' type="text" defaultValue={product ? product.price.toString() : ''} 
                          className="form-control price-input" required onChange={e => handleChange(e)} onKeyPress={(e) => handleKeyPress(e)}
                        />
                      </div>
                    </td>
                  </tr>
          ...
          <Modal.Footer>
            ...
            {product 
            ? <Button name="update-btn" type="button" variant="primary" onClick={() => handleSubmit()}>
                <i className='fas fa-edit'/>  Update
              </Button>
            : <Button name="add-btn" type="button" variant="primary" onClick={() => handleSubmit()}>
