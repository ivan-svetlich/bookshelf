...
const HomePage = () => {
  ...
  const {
    ...
    data: lastUpdates,
    error: updatesError,
  }: BookState = useFetchBooks(bookshelfService.getUpdates, 5);
  const {
    loading: loadingTopBooks,
    data: topBooks,
    error: topBooksError,
  }: BookState = useFetchBooks(bookshelfService.getTopBooks, 3);

  const loading =
    loadingUpdates || loadingTopBooks || !topBooks || !lastUpdates;
