...
export const useSearch = (
  ...
  useEffect(() => {
    const timeoutId = setTimeout(async () => {
      if (args.searchTerm) {
        navigate(
          `/search?q=${encodeURIComponent(args.searchTerm)}&field=${
            args.field
          }&store-only=${storeOnly}&start-index=${args.startIndex}`,
          { replace: true }
        );
      ...
      await googleBooks
        ...
        .then((res) => {
          if (res.data.totalItems > 0) {
            return res.data.items.map((item: BookFromGoogle) =>
              bookMapper(item)
            );
          } else {
            return [];
