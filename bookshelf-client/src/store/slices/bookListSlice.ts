...
>("userListUpdate/update", async (data, thunkAPI) => {
  try {
    if (data.id) {
      ...
      thunkAPI.dispatch(
        setMessage({ content: "Book ID not available", variant: "Danger" })
      );
    ...
    thunkAPI.dispatch(
      setMessage({ content: error.response.data, variant: "Danger" })
    );
...
>("userListUpdate/add", async (data: BookEntry, thunkAPI) => {
  try {
