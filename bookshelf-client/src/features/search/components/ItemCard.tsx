    ...
    <div className="card-border">
      <div className="card flex-row flex-wrap item-card">
        ...
        <div className="item-info">
          <div className="line-clamp-1 card-title" title={item.title}>
            <h2>
              <a href={`/book/${item.id}`}>{item.title}</a>
            </h2>
          </div>
          <div className="line-clamp-1" title={item.authors}>
            <h5>
              <b>Author(s): {item.authors}</b>
            </h5>
          </div>
          <div className="line-clamp-1" title={item.publisher}>
            <p>Publisher: {item.publisher}</p>
          </div>
          <ul className="card-links">
            <li>
              <button className="btn btn-link shadow-none">
                <a href={`/book/${item.id}`}>Details</a>
              </button>
            </li>
