...
const ItemCard = ({topBook, index}: TopBookProps) => {
    ...
    return(
        <div className="card flex-row flex-wrap item-card">
            <div className={`card-header border-0 ranking ${getRankingClass(index)}`} title={topBook.title}>
            ...
            <div className="item-info top-book">
                <div className="line-clamp-1" title={topBook.title}>
                    <a href={`/book/${topBook.googleBooksId}`}><h5>{topBook.title}</h5></a>
                </div>       
                <div className="line-clamp-1" title={topBook.authors ? topBook.authors : 'No data'}>
                    <p><b>Author(s): {topBook.authors}</b></p>
                </div>
