...
const useChatHub = (username: string | undefined) => {
    ...
    const [contactList, setContactList] = useState<ContactListState>({users: [], loading: true});
    ...
    const [alertMessage, setAlertMessage] = useState<Alert>({show: false, content: ''});
    ...
    useEffect(() => {
        if(connected && hubConnection) {
            hubConnection.on('ReceiveMessage', message => {
                if(message.from === selectedUserRef.current || message.to === selectedUserRef.current) {
    ...
    const sendMessage = async (to: string, message: string) => {
        ...
        if (hubConnection && hubConnection.state.valueOf().toUpperCase() === 'CONNECTED') {
            try {
