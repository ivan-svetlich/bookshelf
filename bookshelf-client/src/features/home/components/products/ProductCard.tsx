        ...
        <div className="card flex-row flex-wrap item-card">
            ...
            <div className="item-info">
                <div className="line-clamp-1" title={item.title}>
                    <a href={`/book/${item.googleBooksId}`}><h5><b>{item.title}</b></h5></a></div>       
                <div className="line-clamp-1">
                    <p><b>{statusMapper(item.status)}{item.score ?  ' - score: ' + item.score : ''}</b></p></div>
                <div className='date-user'>
                    <p>{item.updatedAt ? formatDate(item.updatedAt) : '-/-/-'}<span> by </span>
...
const formatDate = (date: Date) => {
    const datetime = DateTime.fromISO(date.toString(), {zone: 'utc'}).setZone().toLocaleString(DateTime.DATETIME_MED);
