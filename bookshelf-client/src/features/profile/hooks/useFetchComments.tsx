...
type ACTION = { type: "LOAD", payload: null } | { type: "SUCCESS"; payload: IComment[] } | { type: "FAILURE", payload: string};

const initialState: CommentState = { loading: false, data: [], error: null };

function fetchCommentsReducer<CommentState>(state: CommentState, action: ACTION) {
...
export const useFetchComments = (
  ...
  username: string, 
  update: boolean, 
  setUpdate: React.Dispatch<React.SetStateAction<boolean>>): CommentState => {
...
async function fetchComments(username: string, fetchResource: Function, dispatch: React.Dispatch<ACTION>) {
  if (username) {
