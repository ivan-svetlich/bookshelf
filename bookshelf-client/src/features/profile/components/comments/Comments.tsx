...
import { getProfileComments, submitComment } from '../../../../utils/api/comments';
...
const Comments = ({username}: CommentsProps) => {
    ...
    const {loading, data, error}: CommentState = useFetchComments(getProfileComments, username, update, setUpdate);
    ...
    return (
        <div id="comments-container" ref={commentsRef} onLoad={() => scrollToComments()}>
            <h4>Comments</h4>
            <Form className="submit-comment" onSubmit={e => handleSubmit(e)}>
                <Form.Group className="mb-3" controlId="exampleForm.ControlTextarea1">
                    <Form.Label>Write a comment</Form.Label>
                    <Form.Control as="textarea" rows={5} onChange={e => handleChange(e as React.ChangeEvent<HTMLInputElement>)} required isInvalid={invalid} value={input} />
            ...
            <div>
                <Table striped className='no-border comments-table'>
                    <tbody className="comment-box">
                        ...
                        {data.length === 0 && <div className='text-is-centered'>No comments yet!</div>}
                    </tbody>
                </Table>
            </div>}
        </div>
