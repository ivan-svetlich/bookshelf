  ...
  return (
      ...
      title={
        <div title="Notifications">
          ...
          {notificationState.data && notificationState.data.length > 0 && (
            <sup className="notif-number"> {notificationState.data.length}</sup>
          )}
        </div>
      }
    ...
    >
        ...
        notificationState.data.map((notification) => {
          if (notification.category === "Friend request") {
            return (
              ...
              >
                <span className="notif-icon-container">
                  <i className="far fa-user notif-icon" />
                </span>
                <a href={`/profile/${notification.reference}`}>
                  {" "}
                  {notification.reference}
                </a>{" "}
                sent you a friend request. <a href={`/friends/pending`}>See</a>
              </NavDropdown.ItemText>
            );
            ...
            return (
              ...
              >
                <span className="notif-icon-container">
                  <i className="far fa-comment notif-icon" />
                </span>
                <a href={`/profile/${notification.reference}`}>
                  {" "}
                  {notification.reference}
      ...
      {notificationState.error &&
        "Sorry! Couldn't load notifications. Please reload the page."}
    </NavDropdown>
  );
};
