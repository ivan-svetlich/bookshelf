...
const AddFriend = () => {
    ...
    let [profileState, username, setUsername] = useSearchUser(query.get("q") ? query.get("q") as string : '', 1000);
    const user: LoginState['user'] = useAppSelector(state => state.login.user);
    const [fiendListState, setUpdate] = useFetchFriendList('all');

    return (
        <div>
            <SubHeader title="Add friend" childComp={() => FriendsNav('add')} icon='fas fa-user-plus'></SubHeader>
            <div id="add-friend-page">
                ...
                <div id="add-friend-results">
                    ...
                    {!profileState.loading && profileState.data && fiendListState.data &&
                    <UserCard user={profileState.data} setUpdate={setUpdate}
                        friendRequestSent={isFriend(profileState.data, fiendListState.data) 
