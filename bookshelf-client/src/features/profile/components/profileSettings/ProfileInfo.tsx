...
const ProfileInfoPage = () => {
    ...
    let [profileState, setInputs, updateProfileInfo] = useUpdateProfileInfo(initialState);
    ...
    useEffect(() => {
        if(profileState.profileInfo) {
            ...
            dispatch(setMessage({content: `Oops! Couldn't update profile info. Please try again`, 
                variant: 'Danger'}));
        }
    }, [dispatch, profileState.error, profileState.profileInfo])

    const birthdayDate: string | undefined = user && user.birthday ? new Date(user.birthday).toISOString().split('T')[0] : undefined;

    return (
        <div className="settings-info-page">
            <SubHeader title="Profile info" childComp={() => SettingsNav('info')} icon='fas fa-user-edit'></SubHeader>
                <div className="form-container">
                <Form onSubmit={handleSubmit}>
                    {user && <div>
                    <Form.Group className="mb-3" controlId="formGridGender">
                        <Form.Label>Gender</Form.Label>
                        <Form.Select defaultValue={user && user.gender ? user.gender : 'non Specified'} name="gender" onChange={(e) => handleChange(e)}>
                    ...
                    <Form.Group className="mb-3" controlId="formGridBirthday">
                        ...
                        <Form.Control defaultValue={birthdayDate} id="datepicker" type="date" name="birthday" onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(e)} />
                    </Form.Group>

                    <Form.Group className="mb-3" controlId="formGridLocation">
                        <Form.Label>Location</Form.Label>
                        <Form.Control defaultValue={user && user.location ? user.location : ''} placeholder="" name="location" onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(e)} />
                    </Form.Group>
                    <div>
