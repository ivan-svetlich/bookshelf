...
const initialState: UpdateProductState = { loading: false, id: null, error: null };
type ACTION = { type: "LOAD", payload: null } | { type: "SUCCESS"; payload: number | null } | { type: "FAILURE", payload: string};

function updateProductReducer<UpdateProductState>(state: UpdateProductState, action: ACTION) {
...
export async function addProduct(product: ProductDTO, dispatch: React.Dispatch<ACTION>) {
  if (product) {
...
export async function updateProduct(update: ProductUpdate, dispatch: React.Dispatch<ACTION>) {
  if (update) {
    dispatch({ type: 'LOAD', payload: null });
    try {
...
export async function removeProduct(productId: number, dispatch: React.Dispatch<ACTION>) {
  if (productId) {
    dispatch({ type: 'LOAD', payload: null });
    try {
