        ...
        {users &&
          ...
          users.map((user: Contact) => (
            <tr
              ...
              className={`contact ${
                user.username === selectedUser ? "" : "enabled"
              }`}
            >
              <td className="col-username">
                <div>
                  ...
                  {user.username === selectedUser && (
                    <i className="fas fa-comments active-chat" />
                  )}
                </div>
              </td>
              <td
                className={`col-new-messages ${
                  user.username === selectedUser
                    ? `td-selected`
                    : `td-new-messages`
                }`}
                ...
                <div>
                  {user.username !== selectedUser && user.newMessages > 0 && (
                    <>
                      ...
                      <sup> +{user.newMessages}</sup>
                    </>
                  )}
                </div>
              </td>
              <td
                className={`col-status td-status ${
                  user.connected ? "is-online" : "is-offline"
                }`}
              >
