  ...
  const { searchState, args, setArgs } = useSearch(
    {
      ...
      filter: "partial",
      startIndex: parseInt(
        query.get("start-index") ? (query.get("start-index") as string) : "0"
      ),
    },
    storeOnly,
    500
    ...
    <div>
      ...
      <div id="search-page">
        ...
        <div id="item-cards">
          ...
          {data &&
            data.length > 0 &&
            data.map((item: Book) => <ItemCard item={item} />)}
        </div>
        {args.searchTerm && data && data.length === 0 && "No results"}
        {data && data.length > 0 && (
          <Paginator args={args} handleChange={setArgs} data={data} />
        )}
      </div>
    </div>
  );
