...
import useUpdateProduct, { addProduct, removeProduct, updateProduct } from "../hooks/useAddProduct";
...
const ItemPage = () => {
  ...
  const {loading, data, error}: BookState = useFetchBook(googleBooksService.getBookById, id);
  ...
  return(
    <div>
      <SubHeader title={data ? `${data.title}, by ${data.authors}` : "Loading..."} icon='fas fa-book'></SubHeader>
      <div id="item-page">
          ...
          {data && !loading && !loadingProduct && <>
            <div className="card flex-row flex-wrap item-card">
                ...
                <div className="admin-btn">
                  <div className="admin-attribution"><i className="fas fa-user-lock"/> Admin attribution</div>
                  {!addProductState.loading && !product &&
                  <Button variant="outline-dark" onClick={handleShowUpdateProduct}>
                  ...
                  {addProductState.error && <span className="add-failure">{addProductState.error}</span>}
                </div>
                }
