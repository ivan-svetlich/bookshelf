  ...
  useEffect(() => {
    if (chat.length !== messageCount) {
      if (
        !scrollEnd &&
        chat.length > 0 &&
        chat[chat.length - 1].to === user?.username
      ) {
    ...
    <>
      ...
      <div className="msg-window" onScroll={(e) => handleScroll(e)}>
        {selectedUser &&
          chat.map(
            (m) =>
              ((m.from === user?.username && !m.deletedBySender) ||
                (m.to === user?.username && !m.deletedByReceiver)) && (
      ...
      <div className="new-msg-container">
        {newMessage > 0 && (
          ...
          >
            ...
            {newMessage > 1 && `${newMessage} New messages`}{" "}
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          Are you sure you want to delete all messages between you and{" "}
          {selectedUser}?
        </Modal.Body>
        <Modal.Footer>
