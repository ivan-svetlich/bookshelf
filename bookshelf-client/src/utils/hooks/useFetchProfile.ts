...
type UseFetchProfileReturnArgs = [ProfileState, React.Dispatch<React.SetStateAction<boolean>>];

export const useFetchProfile = (
  fetchProfileInfo: Function, 
  fetchBooklist: Function,
  username: string | undefined): UseFetchProfileReturnArgs => {
...
async function fetchProfile(username: string, fetchProfileInfo: Function, fetchBooklist: Function, 
  ...
  if (username) {
    ...
    try {
      ...
      dispatch({ type: 'BOOKLIST_SUCCESS', payload: booklist.data as BookEntry[] })  
    }
    catch (error: any) {
