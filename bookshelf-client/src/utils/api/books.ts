...
const getAll = (username: string) => {
  return axiosInstance.get(BOOKS + `/?username=${username}`, {
    headers: { Authorization: `Bearer ${getCookie("id_token")}` },
  });
};

const getPurchasedBooks = () => {
  return axiosInstance.get(BOOKS + `/Purchased`, {
    headers: { Authorization: `Bearer ${getCookie("id_token")}` },
  });
};
...
const post = async (data: BookEntry) => {
  ...
  return axiosInstance.post(BOOKS, body, {
    headers: { Authorization: `Bearer ${getCookie("id_token")}` },
  });
};

const update = async (itemId: number, body: BookEntry) => {
  return axiosInstance.put(BOOKS + `/${itemId}`, body, {
    headers: { Authorization: `Bearer ${getCookie("id_token")}` },
  });
};

const getById = async (itemId: number) => {
  return axiosInstance.get(BOOKS + `/${itemId}`, {
    headers: { Authorization: `Bearer ${getCookie("id_token")}` },
  });
};

const getFile = async (type: string) => {
  return axiosInstance.get(BOOKS + `/File?type=${type}`, {
    headers: { Authorization: `Bearer ${getCookie("id_token")}` },
  });
};
const remove = async (itemId: number) => {
  return axiosInstance.delete(BOOKS + `/${itemId}`, {
    headers: { Authorization: `Bearer ${getCookie("id_token")}` },
  });
};
